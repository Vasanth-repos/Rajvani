...
- **Code-Switching Representation & Empirical WER Gap [Provisional: Insufficient Sample Size (N=6 < 20)]**: In the held-out test suite (`data/realworld_test_200.jsonl`), only 6 of 200 utterances (3.0%) contain tagged English/Hindi code-switching spans. Empirical evaluation reveals a **+2.60 pts WER gap** on code-switched records (8.29% WER, 95% CI: [1.39%, 15.44%]) versus clean monolingual records (5.70% WER, 95% CI: [4.75%, 6.68%]). Because $N=6$ is below the project's minimum reliability threshold ($N \ge 20$), this delta is strictly classified as **Provisional / Insufficient Data** pending dedicated code-switched test set expansion via `codeswitch/cs_eval_set_builder.py`.
